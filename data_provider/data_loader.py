...
class Dataset_task(Dataset):
    def __init__(self, root_path="order book data/", flag='train', size=None,
                 ...
                 product="cu", vali_set=["202201", "202202"], train_set=["202202", "202204"]):
        self.seq_len = size[0]
    ...
    def __read_data__(self):
        ...
        vali_set = all_dates[(np.array(all_dates) > self.vali_set[0]) & (np.array(all_dates) < self.vali_set[1])]
        train_set = all_dates[(np.array(all_dates) > self.train_set[0]) & (np.array(all_dates) < self.train_set[1])]
