...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        if isinstance(kernel_size, list): # 若args.moving_avg为列表
            self.decomp = series_decomp_multi(kernel_size) # 初始化SeriesDecomp对象
        else:
        ...
        self.encoder = Encoder(
            [
                EncoderLayer(AutoCorrelationLayer(encoder_self_att,
                                                  configs.d_model,
                                                  configs.n_heads),
                             configs.d_model,
                             configs.d_ff,
        ...
        self.decoder = Decoder(
            [
                DecoderLayer(AutoCorrelationLayer(decoder_self_att,
                                                  configs.d_model,
                                                  configs.n_heads),
                             AutoCorrelationLayer(decoder_cross_att,
                                                  configs.d_model,
                                                  configs.n_heads),
                             configs.d_model,
                             configs.c_out,
